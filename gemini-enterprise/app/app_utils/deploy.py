...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Delete existing agent and create from scratch (required to change networking)",
)
def deploy_agent_engine_app(
    ...
    # Use v1beta1 API when agent identity or network attachment is enabled (Preview features)
    ...
    if matching_agents and recreate:
        click.echo(f"🗑️ Deleting existing agent: {display_name} (required for network updates)...")
        delete_op = client.agent_engines.delete(name=matching_agents[0].api_resource.name, force=True)
        _agent_engines_utils._await_operation(
            operation_name=delete_op.name,
